...
//            imageView = (ImageView) view.findViewById(R.id.albumArt);
    ...
    public FavoritesAdapter(ArrayList<Song> songList,Context context,OnItemClickListener listener) {
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        if (song.getTitle().length() > 30) holder.title.setText(song.getTitle().substring(0,30));
        else holder.title.setText(song.getTitle());
        if (song.getArtist().length() > 30) holder.artist.setText(song.getArtist().substring(0,30));
